...
class City_selection_chenge:
    ...
    def message_state_city_selection(message):
        ...
        if message.html_text in all_city:
            app.User_City(message.chat.id).add_city(message.html_text)
        ...
        if message.html_text == 'стоп':
            app.User_botgo(message.chat.id).query_change_state("age_category")
            ...
            log.log(message.chat.id, "the user has selected all the cities that interest him,  \
    ...
    def message_state_city_change(message):
        ...
        all_city = sorted(set(app.Cities(message.chat.id).get_all_cities()) - set(listCity))
        for city in all_city:
            towns.add(types.KeyboardButton(city))
